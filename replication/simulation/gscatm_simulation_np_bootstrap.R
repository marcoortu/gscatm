...
# Non-parametric bootstrap version (row-resampling + percentile CIs)
#
# Key differences vs gscatm_simulation_benchmark.R:
#   - Non-parametric row-bootstrap: (dtm_i, x_i) pairs resampled with replacement
#   - Percentile CIs (2.5%/97.5% quantiles of bootstrap distribution)
#   - n_docs = 1000, P = 5  (restored to pre-review values for reliable RMSE)
#
# Why non-parametric bootstrap recovers coverage:
#   The parametric bootstrap resamples word counts from fixed (theta_hat, phi_hat),
#   capturing only word-count variability — negligible when documents are long.
#   This produces near-zero bootstrap SEs and therefore near-zero coverage of B_true.
#   The non-parametric row-bootstrap resamples entire (document, covariate) pairs,
#   capturing BOTH latent theta variability and word-count variability, which is the
#   correct target for inference on the population regression coefficients B.
...
# ---- Helper: topic matching (label switching) ----
match_topics <- function(phi_true, phi_hat) {
  K_true <- nrow(phi_true)
  K_hat  <- nrow(phi_hat)
  if (K_true != K_hat) stop("phi_true and phi_hat must have same number of rows.")
...
# ---- Helper: ALR-WLS regression for covariate effects ----
estimate_alr_effects <- function(theta, X_std, reference_topic = ncol(theta)) {
  ...
  topic_idx <- c(setdiff(1:K, reference_topic), reference_topic)
  ...
  if (is.null(colnames(X_std))) colnames(X_std) <- paste0("x", 1:P)

  B_hat  <- matrix(NA_real_, nrow = P, ncol = K - 1,
                   dimnames = list(colnames(X_std), paste0("Topic_", 1:(K - 1))))
  ...
  for (k in 1:(K - 1)) {
    ...
    keep     <- rownames(ct) %in% colnames(X_std)
    ct       <- ct[keep, , drop = FALSE]
    B_hat[rownames(ct), k]  <- ct[, "Estimate"]
    se_hat[rownames(ct), k] <- ct[, "Std. Error"]
  }
  list(B_hat = B_hat, se_hat = se_hat)
}

# ---- Helper: convert dgCMatrix to stm format ----
dtm_to_stm <- function(dtm) {
  ...
  for (i in 1:N) {
    ...
    if (length(idx) == 0) {
      ...
      documents[[i]] <- rbind(idx, as.integer(dtm[i, idx]))
    }
  }
...
# ---- Fit models and evaluate metrics for one dataset ----
evaluate_method <- function(sim,
  ...
  if (method == "gscatm") {
    ...
    phi_hat_raw   <- exp(logbeta) / rowSums(exp(logbeta))
  ...
  # --- Non-parametric bootstrap coverage (gscatm only) ---
  # Resample (document, covariate) rows with replacement.
  # This captures both sources of variability in B_hat:
  #   (1) latent theta variability (ALR noise in the DGP)
  #   (2) word-count sampling variability
  # Coverage is measured via percentile CI [Q_0.025, Q_0.975] of the
  # bootstrap distribution of B_hat^(b) — invariant to bias, no symmetry
  # assumption, and consistent with the non-parametric bootstrap literature.
  ...
  if (isTRUE(bootstrap_ci) && method == "gscatm") {
    ...
    for (b in seq_len(n_bootstrap)) {
      tryCatch({
        # Step 1: resample documents with replacement
        ...
        # Step 2: refit GSCA-TM (no nested bootstrap)
        ...
        # Step 3: align bootstrap topics to original phi_hat (not phi_true)
        perm_b  <- match_topics(phi_hat, fit_b$phi)
        theta_b <- fit_b$theta[, perm_b, drop = FALSE]

        # Step 4: ALR-WLS on bootstrap sample using bootstrap X_std
    ...
    boot_lo <- apply(boot_B_arr, c(1L, 2L), quantile,
                     probs = 0.025, na.rm = TRUE)
    boot_hi <- apply(boot_B_arr, c(1L, 2L), quantile,
...
clusterExport(cl, varlist = c(
  ...
  "compute_perplexity", "match_topics", "estimate_alr_effects", "dtm_to_stm",
